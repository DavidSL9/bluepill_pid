...
long p_samples[SAMPLES];
long u_samples[SAMPLES];
...
static void mainTask(void *pvParameters)
{
  ...
  while(1)
  {
    ...
    int out = int(3 * pid.output());
    writeOutput(out);

    if (idx < SAMPLES)
    {
      p_samples[idx++] = response;
      u_samples[idx] = pid.output();
    }
    vTaskDelay(1);
  }
}
